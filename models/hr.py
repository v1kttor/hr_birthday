...
class Birthday(models.Model):
    ...
    birthday_date = fields.Date(compute="_days_left", store=True)
...
class Department(models.Model):
    ...
    def _cron_check_birthdays(self):
        if check_birthdays == True:
            return True
        if birthday_remind_days == True:
            return True
        else:
            pass # create event
...
# of days. If there are, a birthday event (hr.birthday) record
